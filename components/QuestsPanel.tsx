    ...
    <div className="flex-1 p-6 overflow-y-auto">
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        {PERIODS.map((period) => {
          ...
          return (
            <div key={period.key} className="rounded-xl border border-slate-800 bg-slate-950/60 p-4">
              ...
              <div className="mt-4 grid gap-2 sm:grid-cols-2">
                <div className="rounded-lg border border-slate-800 bg-slate-900 px-3 py-2 text-xs text-slate-300">
                  Reward: {formatMoney(quest?.reward ?? 0)} NVC
                </div>
                <div className="rounded-lg border border-slate-800 bg-slate-900 px-3 py-2 text-xs text-slate-300">XP: +{quest?.xpReward ?? 0}</div>
              </div>
